...
public class WorkLogReportTest {
    ...
    @Test
    public void testAddOfWorkLogEntries() {
        WorkLogReport workLogReport = new WorkLogReport();

        workLogReport.getWorkLogEntryList().add(
                WorkLogEntryFixture.getWorkLogEntryWithData());

        assertEquals(1, workLogReport.getWorkLogEntryList().size());

        workLogReport.getWorkLogEntryList().add(
                WorkLogEntryFixture.getWorkLogEntryWithData());

        assertEquals(2, workLogReport.getWorkLogEntryList().size());
    }
